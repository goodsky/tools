...
def read_podcast_episodes_from_rss(rss_url) -> dict:
    try:
...
def download_audio(episode, output_dir):
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download audio files from an RSS feed.")

    parser.add_argument("rss_url", help="The URL of the RSS feed.")
    parser.add_argument("-o", "--output", default="downloads", help="The directory to save downloaded files (default: downloads).")
    ...
    episodes = read_podcast_episodes_from_rss(rss_url)
    if not episodes:
